...
def plt_depth_on_im(depth_map, im, s=2, title=''):
    ...
    '''plt.figure()
    plt.imshow(im) 
    plt.scatter(x_map[msk], y_map[msk], c=depth_map[msk], s=2, cmap='jet')
    plt.axis('off')
    plt.colorbar()
    plt.title(title)'''
    ...
    ax = plt.axes()
    image = ax.imshow(im)
    pc = ax.scatter(x_map[msk], y_map[msk], c=depth_map[msk], s=s, cmap='turbo', vmin=0, vmax=100)
    ax.axis('off')
    divider = make_axes_locatable(plt.gca())
    cax = divider.append_axes("right", "2%", pad="1%")
    cbar = fig.colorbar(pc, cax=cax, orientation="vertical", ax=ax) # Similar to fig.colorbar(im, cax = cax)
    cbar.ax.tick_params(labelsize=14)
    cbar.set_label('Tiefe in m', rotation=270, labelpad=18, size=14)
    plt.tight_layout()
...
def plt_flow_on_im(flow, im, skip = 3, title=''):
    ...
    vec_abs = np.linalg.norm(np.array(flow), axis=2)
    norm = matplotlib.colors.Normalize()
    norm.autoscale(vec_abs)
    vec_abs = norm(vec_abs)
    
    for i in range(0, h, skip+1):
        for j in range(0, w, skip+1):
            if msk[i,j]:
                color = vec_abs[i,j]
                plt.arrow(j,i, flow[i,j,0], flow[i,j,1], length_includes_head=True, width=0.05, head_width=0.5, color=colormap(color))
    divider = make_axes_locatable(plt.gca())
    cax = divider.append_axes("right", "2%", pad="1%")
    m = matplotlib.cm.ScalarMappable(cmap=colormap, norm=norm)
    m.set_array([])
    cbar = fig.colorbar(m, cax=cax, orientation="vertical", ax=ax) # Similar to fig.colorbar(im, cax = cax)
    cbar.ax.tick_params(labelsize=14)
    cbar.set_label('Vektorbetrag in m/s', rotation=270, labelpad=18, size=14)
    plt.tight_layout()
    #plt.title(title)
...
def lidarFlow2uv(flow, K, depth_map, downsample_scale=2, y_cutoff=34):
    '''
    uv_map: h x w x 2
    '''
    f = K[0,0]
...
def cal_uv1(h, w, K, downsample_scale=2, y_cutoff=34):
    '''
    uv_map: h x w x 2
